...
function Report() {
  ...
  const fetchReports = () => {
    ...
    axios
      ...
      .then((response) => {
        console.log("Fetched reports:", JSON.stringify(response.data, null, 2)); // Add this
  ...
  const viewDetails = (report) => {
    console.log("Received report data:", {
      firstItem: report.reportData[0],
      prices: report.reportData.map(i => ({
        name: i.name,
        purchasePrice: i.purchasePrice,
        salePrice: i.salePrice
      }))
    });
    console.log("Selected report data:", JSON.stringify(selectedReport, null, 2));
    setSelectedReport(report);
    setShowDetails(true);
  };
// In your branch manager's Report component
const sendToAdmin = async (reportId) => {
  try {
    const response = await axios.post(`http://localhost:3001/reports/${reportId}/send-to-admin`, {
      sentToAdmin: true,
      sentAt: new Date() // Optional: track when report was sent
    });
  ...
  >
    {report.sentToAdmin ? 'Sent' : 'Send to Admin'}
              ...
              <tr>
                <td colSpan="6" className="px-4 py-2 text-center text-gray-400">No reports available</td>
              </tr>
            )}
          </tbody>
        </table>
    ...
    <div className="bg-gray-800 rounded-lg shadow-xl p-6 max-w-4xl w-full max-h-screen overflow-auto">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-2xl font-semibold text-blue-400">
          Product Details: {new Date(selectedReport.startDate).toLocaleDateString()} - {new Date(selectedReport.endDate).toLocaleDateString()}
        </h3>
        <button 
